    ...
    def stop(self) -> None:
        ...
        if self.vr_streaming_server:
            try:
                ...
                if stream_comp and hasattr(stream_comp, 'frame_ready') and self._vr_frame_handler:
                    try:
                        stream_comp.frame_ready.disconnect(self._vr_frame_handler)
                    except Exception:
                        pass
                    self._vr_frame_handler = None

                if stream_comp and hasattr(stream_comp, 'set_vr_capture_enabled'):
                    try:
                        stream_comp.set_vr_capture_enabled(False)
                    except Exception:
                        pass
                elif stream_comp and hasattr(stream_comp, '_vr_capture_enabled'):
                    stream_comp._vr_capture_enabled = False
