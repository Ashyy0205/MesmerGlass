...
from PyQt6.QtWidgets import (
    ...
    QGroupBox, QHBoxLayout, QColorDialog
)
...
class TextFxPage(QWidget):
    ...
    loadFontRequested = pyqtSignal()  # signal for new in-tab font load

    def __init__(self, *, text: str, text_scale_pct: int, fx_mode: str, fx_intensity: int,
                 flash_interval_ms: int, flash_width_ms: int, parent=None):
        ...
        hl.setSpacing(8)
        lab = QLabel("Current pack")
        lab.setMinimumWidth(110)
        self.lab_pack_name = QLabel("(none)")
        btn_load = QPushButton("Load…")
        btn_load.setToolTip("Load a message pack (weighted random per flash)")
        btn_create = QPushButton("Create Pack…")
        btn_create.setToolTip("Open pack editor")
        hl.addWidget(lab)
        hl.addWidget(self.lab_pack_name, 1)
        ...
        cv = QVBoxLayout(card_txtfx)
        cv.setContentsMargins(12, 10, 12, 12)
        cv.setSpacing(4)
        self.lab_text = QLabel(text)
        self.lab_text.hide()
        self._color_value = "#FFFFFF"
        self.btn_color = QPushButton("Text Colour…")
        self.lab_color_preview = QLabel(self._color_value)
        self.lab_color_preview.setMinimumWidth(70)
        ...
        # Font family row (with in-tab Load Font button)
        self.lab_font_family = QLabel("(default)")
        self.btn_load_font = QPushButton("Load Font…")
        font_row = QWidget()
        frl = QHBoxLayout(font_row)
        frl.setContentsMargins(8, 4, 8, 4)
        frl.setSpacing(8)
        lab_font = QLabel("Font family")
        lab_font.setMinimumWidth(140)
        frl.addWidget(lab_font)
        frl.addWidget(self.lab_font_family, 1)
        frl.addWidget(self.btn_load_font, 0)
        cv.addWidget(font_row)
        ...
        cv.addWidget(_row("Size (% height)", self.sld_txt_scale, self.lab_txt_scale))
        ...
        fv.setSpacing(4)
        self.spin_interval = UnitSpin(200, 10000, flash_interval_ms, "ms", step=50, tooltip="Time between flashes", width=160)
        fv.addWidget(_row("Interval", self.spin_interval))
        self.spin_width = UnitSpin(50, 3000, flash_width_ms, "ms", step=25, tooltip="Flash visible duration", width=160)
        ...
        try:  # Qt may not be fully available in headless test mode
            self.btn_load_font.clicked.connect(self.loadFontRequested.emit)
        except Exception:  # pragma: no cover
            pass

        # Keep references for tests if needed
        self._btn_load_pack = btn_load
        self._btn_create_pack = btn_create
    ...
    def set_pack_name(self, name: str | None):
        self.lab_pack_name.setText(name or "(none)")

    def update_font_label(self, fam: str | None):
        try:
            self.lab_font_family.setText(fam or "(default)")
        except Exception:
            pass
    ...
    # Legacy compatibility helpers expected elsewhere
    def is_auto_cycle_enabled(self) -> bool:
        return True

    def cycle_interval_secs(self) -> int:
        return 5
    ...
    def _choose_color(self):
        try:
            ...
            if not col.isValid():
                return
            self._color_value = col.name()
            self.lab_color_preview.setText(self._color_value)
            self.lab_color_preview.setStyleSheet(
                f"background:{self._color_value}; border:1px solid #555; padding:2px;"
            )
