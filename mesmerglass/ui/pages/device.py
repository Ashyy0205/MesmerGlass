...
class DevicePage(QWidget):
    ...
    deviceSelected = pyqtSignal(int)     # Single device index selected
    devicesSelected = pyqtSignal(object) # List[int] of device indices (multi-select)
    ...
    def __init__(
        ...
        *,
        ...
        scan_layout.addWidget(self.device_label, 1)

        cv.addWidget(scan_widget)
        root.addWidget(card_enable)

        # 2) Buzz-on-flash bubble
        card_buzz = _card("Buzz on flash")
        bv = QVBoxLayout(card_buzz)
        ...
        self.sld_buzz = QSlider(Qt.Orientation.Horizontal)
        self.sld_buzz.setRange(0, 100)
        self.sld_buzz.setValue(buzz_intensity_pct)
        ...
        # 3) Random bursts bubble
        card_bursts = _card("Random bursts")
        rv = QVBoxLayout(card_bursts)
        ...
        self.spin_min = UnitSpin(5, 300, min_gap_s, "s", step=1, tooltip="Minimum time between bursts.", width=140)
        rv.addWidget(_row("Minimum gap", self.spin_min))

        self.spin_max = UnitSpin(6, 600, max_gap_s, "s", step=1, tooltip="Maximum time between bursts.", width=140)
        rv.addWidget(_row("Maximum gap", self.spin_max))

        self.sld_peak = QSlider(Qt.Orientation.Horizontal)
        self.sld_peak.setRange(10, 100)
        self.sld_peak.setValue(peak_pct)
        self.lab_peak = _pct_label(self.sld_peak.value())
        rv.addWidget(_row("Peak level", self.sld_peak, self.lab_peak))

        self.spin_max_ms = UnitSpin(200, 8000, max_ms, "ms", step=50, tooltip="Maximum duration per burst envelope.", width=160)
    ...
    def _on_select_clicked(self):
        ...
        dialog.devicesSelected.connect(self._on_devices_selected)
        dialog.exec()
    ...
    def update_device_list(self, device_list: "DeviceList"):
        ...
        if sel_bp is not None:
            ...
            if selected_device is not None:
                log.info("Using device: %s (index %s)", selected_device.name, sel_bp)
                self.device_label.setText(f"Using: {selected_device.name}")
                self.device_label.setStyleSheet("color: green;")
        elif getattr(device_list, "selected_idx", None) is not None:
            # Fallback to server-style selected_idx (list index) if present
            sel_idx = getattr(device_list, "selected_idx", None)
            if isinstance(sel_idx, int) and 0 <= sel_idx < len(device_list.devices):
                selected_device = device_list.devices[sel_idx]
                log.info("Using device: %s (list idx %s)", selected_device.name, sel_idx)
                self.device_label.setText(f"Using: {selected_device.name}")
                self.device_label.setStyleSheet("color: green;")
        else:
